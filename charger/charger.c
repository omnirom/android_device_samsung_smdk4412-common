...
static int read_file_int(const char * path, int * val) {
	...
	if (end == buf || ((end < buf + sizeof(buf)) && ( * end != '\n' && * end != '\0'))) goto err;
...
static int init_supplies(struct charger * charger) {
	...
	if (ac != -1 || usb != -1) {
		...
		if (online_ac && ac != -1) charger - > charger_type = AC;
		else if (online_usb && usb != -1) charger - > charger_type = USB;
	} else {
...
static int update_supply(struct charger * charger) {
	...
	if (supply == AC) path = supplies_list - > ac_online_path;
	else if (supply == USB) path = supplies_list - > usb_online_path;
...
static int request_suspend(bool enable) {
	int fd;
	if (enable) {
		gr_fb_blank(true);#ifdef CHARGER_ENABLE_SUSPEND
		return autosuspend_enable();#elif defined DIM_SCREEN && defined BRIGHTNESS_PATH
		fd = open(BRIGHTNESS_PATH, O_WRONLY);
		write(fd, "20", strlen("20"));
		close(fd);
		return 0;#endif
	}
	gr_fb_blank(false);#ifdef CHARGER_ENABLE_SUSPEND
	return autosuspend_disable();#elif defined DIM_SCREEN && defined BRIGHTNESS_PATH
	fd = open(BRIGHTNESS_PATH, O_WRONLY);
...
static int draw_text(const char * str, int x, int y) {
	...
	if (x < 0) x = (gr_fb_width() - str_len_px) / 2;
	if (y < 0) y = (gr_fb_height() - char_height) / 2;
	gr_text(x, y, str, 0);
...
static void redraw_screen(struct charger * charger) {
	...
	if (batt_anim - > capacity < 0 || batt_anim - > num_frames == 0) draw_unknown(charger);
	else {
...
static void update_screen_state(struct charger * charger, int64_t now) {
	...
	if (batt_anim - > cur_cycle == batt_anim - > num_cycles) {
		...
		if (charger - > online > 0) request_suspend(true);
		return;
	}
	...
	if (batt_anim - > cur_frame == 0) {
		...
		if (batt_cap >= 0 && batt_anim - > num_frames != 0) {
			...
			for (i = 1; i < batt_anim - > num_frames; i++) {
				if (batt_cap < batt_anim - > frames[i].min_capacity) break;
			}
	...
	if (batt_anim - > cur_cycle == 0) gr_fb_blank(false);
	 ...
	 */
	while (batt_anim - > cur_frame < batt_anim - > num_frames && batt_anim - > frames[batt_anim - > cur_frame].level_only)
	batt_anim - > cur_frame++;
...
int64_t timeout) {
	...
	if (charger - > next_key_check == -1 || then < charger - > next_key_check) charger - > next_key_check = then;
}
...
static void handle_input_state(struct charger * charger, int64_t now) {
	...
	if (charger - > next_key_check != -1 && now > charger - > next_key_check) charger - > next_key_check = -1;
}
...
static void wait_next_event(struct charger * charger, int64_t now) {
	...
	if (charger - > next_key_check != -1 && charger - > next_key_check < next_event) next_event = charger - > next_key_check;
	if (next_event != -1 && next_event != INT64_MAX) timeout = max(0, next_event - now);
	else timeout = -1;
